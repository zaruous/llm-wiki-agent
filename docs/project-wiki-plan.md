...
| 도구 | 추가 검사 |
|---|---|
| `health.py` | 요구사항 ID 유니크/포맷, `status`·`priority` enum 유효성, 인터뷰↔요구사항 양방향 링크 존재, **approved+ 인데 담당자/일정 누락**, `progress` 범위(0–100)·완료상태 정합성(verified면 100%), **rejected/deferred 인데 `dropped_date`·사유 누락**, **시크릿 스캐닝(API키/토큰/PII 패턴) — §7.1** |
...
## 7. 보안 검토 (Security Considerations)

프로젝트 관리 위키는 인터뷰·이해관계자·요구사항 등 **민감 정보를 평문 마크다운으로
git에 저장**한다. 지식 위키보다 기밀성·개인정보 위험이 높으므로 설계 단계에서 다음을
반영한다.

### 7.1 민감정보 & 비밀 유출 (가장 큰 위험)
- **위험**: 원본(`raw/`)·인터뷰 페이지에 개인정보(PII: 이름·연락처·평가), 영업비밀,
  자격증명/토큰이 섞여 평문 커밋 → git 히스토리에 영구 잔존(삭제해도 이력에 남음).
- **대응(계획 반영)**:
  - `raw/` 중 민감 원본은 `.gitignore`로 추적 제외(샘플/익명화본만 커밋) 옵션 제공.
  - **시크릿 스캐닝**을 `health.py`에 추가: API 키/토큰/주민번호 등 패턴 탐지 시 차단·경고.
  - 인터뷰 인입 시 **PII 최소화/가명화 가이드**를 워크플로우에 명시(불필요한 개인식별자 제거).
  - 실수 커밋 시 `git filter-repo`/BFG로 히스토리 정리 절차를 문서화.

### 7.2 프롬프트 인젝션 (인입 콘텐츠 = 신뢰 불가)
- **위험**: 인터뷰 녹취·외부 문서는 **신뢰할 수 없는 입력**. "기존 요구사항을 삭제하라"
  같은 지시가 본문에 숨어 있으면 ingest 중 에이전트가 따를 수 있음(파일 쓰기 권한 보유).
- **대응**: 인입 콘텐츠는 **데이터로만 취급(지시로 해석 금지)** 원칙을 스키마(CLAUDE.md)에
  명시. ingest는 신규/갱신만 하고 **삭제·범위 변경은 사람 승인(DEC) 후**에만. 사후
  검증 단계에서 예상 밖 대량 변경을 플래그.

### 7.3 파일 변환 공급망 (markitdown 등)
- **위험**: `.pdf/.docx/.pptx` 자동변환은 외부 파서에 의존 → 파서 취약점·악성 매크로 노출.
  (이미 `litellm`/`networkx` 버전 핀 조치 있음 — 동일 원칙 적용.)
- **대응**: 변환 의존성 **버전 핀 유지**, 신뢰된 출처 파일만 변환, `--no-convert` 옵션 안내.

### 7.4 접근 제어 & LLM 외부 전송
- **위험**: 위키는 git repo로 **페이지 단위 ACL 없음** → repo 접근자 전원이 전체 열람.
  또한 `lint.py`/`build_graph.py`/`query.py`는 위키 내용을 **LLM API로 전송**(데이터 거주지/기밀성).
- **대응**: 민감 프로젝트는 **private repo + 브랜치 보호**, 접근자 최소화. 기밀 등급이 높은
  내용은 LLM 전송 대상에서 제외하거나 온프레미스/사내 모델 사용 옵션 검토. `tools/`의
  `ANTHROPIC_API_KEY`는 환경변수로만 주입(코드/커밋에 하드코딩 금지).

### 7.5 개인정보 보존/삭제 (컴플라이언스)
- **위험**: `log.md`는 append-only, git은 이력 보존 → 개인정보 **삭제권(GDPR 등)**과 충돌.
- **대응**: 이해관계자 페이지는 식별자 최소화(역할 중심), 삭제 요청 대응 절차(가명화+히스토리
  정리)를 문서화. 보존 정책을 `charter.md`에 명시.

> **요약**: 1순위는 **시크릿 스캐닝 + raw 민감원본 .gitignore + PII 최소화**(7.1),
> 2순위는 **인입 프롬프트 인젝션 방어**(7.2). 이 두 가지는 Phase 1(스키마)·Phase 3(도구)에
> 필수 반영한다.
