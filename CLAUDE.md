...
| Command | What to say |
|---|---|
| `/wiki-ingest` | `ingest raw/interviews/2026-06-13-kickoff.md` |
| `/wiki-query` | `query: open must requirements?` |
| `/wiki-health` | `health` (fast, every session) |
| `/wiki-format` | `format the wiki` (normalize frontmatter + check tags) |
| `/wiki-lint` | `lint the wiki` (expensive, periodic) |
| `/wiki-graph` | `build the traceability graph` |
| `/project-status` | `project status` (progress / scope / risks dashboard) |
...
Triggered by: *"format the wiki"* or `/wiki-format`
